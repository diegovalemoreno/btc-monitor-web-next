...
const CONFIDENCE_META: Record<Confidence, { label: string; color: string }> = {
  LOW:    { label: 'Convicção baixa',   color: 'var(--text-muted)' },
  MEDIUM: { label: 'Convicção média',   color: 'var(--text-sec)' },
...
function ScoreBar({ label, value, color }: { label: string; value: number; color: string }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
      ...
      <div style={{ flex: 1, background: 'var(--surface3)', borderRadius: '3px', height: '5px', overflow: 'hidden' }}>
        <div style={{ width: `${pct}%`, height: '5px', background: color, borderRadius: '3px' }} />
      </div>
      <span style={{ fontSize: '11px', color: 'var(--text-sec)', width: '28px', textAlign: 'right' }}>{value}</span>
    </div>
  )
}
...
export default function RecommendationCard({ rec }: { rec: RecommendationData }) {
  ...
  return (
    <div style={{ background: 'var(--surface)', border: `1px solid ${meta.color}33`, borderRadius: '12px', overflow: 'hidden', marginBottom: '40px' }}>
      ...
      <div style={{ padding: '24px', background: meta.bg, borderBottom: `1px solid ${meta.color}22`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <div style={{ fontSize: '12px', color: 'var(--text-sec)', marginTop: '2px' }}>{meta.desc}</div>
        </div>
        <div style={{
          padding:      '4px 12px',
          background:   'var(--bg)',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', borderBottom: '1px solid var(--border-dim)' }}>
        <div style={{ padding: '20px 24px', borderRight: '1px solid var(--border-dim)' }}>
          <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Aportar agora</div>
          <div style={{ fontSize: '24px', fontWeight: 700, color: 'var(--text)' }}>{formatBRL(rec.recommended_amount_brl)}</div>
        </div>
        <div style={{ padding: '20px 24px' }}>
          <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Manter em reserva</div>
          <div style={{ fontSize: '24px', fontWeight: 700, color: 'var(--text-sec)' }}>{formatBRL(rec.reserve_amount_brl)}</div>
        </div>
      </div>

      {/* Rationale */}
      <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border-dim)' }}>
        <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Racional</div>
        <p style={{ margin: 0, fontSize: '13px', color: 'var(--text-sec)', lineHeight: 1.7 }}>{rec.rationale}</p>
      </div>

      {/* Context scores */}
      {(ctx.opportunityScore !== undefined || ctx.riskScore !== undefined) && (
        <div style={{ padding: '20px 24px' }}>
          <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '12px', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Contexto de mercado</div>
